...
@section('content')
    <div class="container mt-5">
        ...
        <div id="characters-table-container">
            @include('management.characters.partials.characters_table', ['characters' => $characters])
        </div>

        <!-- Pagination Links -->
        <div class="d-flex justify-content-end mt-4">
            {{ $characters->appends(['search' => request('search')])->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function performSearch(query) {
                ...
                const url = "{{ route('characters.index') }}?search=" + encodeURIComponent(query);
                ...
                fetch(url, {
                    ...
                    .catch(error => {
                        console.error('Error during AJAX request:', error); // Handle errors
                    })
                    .finally(() => {
